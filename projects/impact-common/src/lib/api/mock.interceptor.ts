...
export class HttpMockApiInterceptor implements HttpInterceptor {
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        return currentMockEndpoint ? (request.method === 'POST' || request.method === 'PUT') ? currentMockEndpoint.handler(request.body) :currentMockEndpoint.handler(request.params) : next.handle(request);
    }
}
